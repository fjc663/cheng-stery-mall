...
public class AdminGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            ...
            // 获得令牌里的用户id信息
             userId = claims.get(JwtClaimConstant.ADMIN_ID, Long.class);
        }catch (Exception e){
            // 拦截，设置响应状态码为401
    ...
    private boolean isExclude(String path) {
        ...
        // 拦截
        for (String pathPatten : authProperties.getAdminIncludePaths()) {
        ...
        // 排除
        for (String pathPatten : authProperties.getAdminExcludePaths()) {
            if (antPathMatcher.match(pathPatten, path)){
                isIntercept = true;
            }
        }
